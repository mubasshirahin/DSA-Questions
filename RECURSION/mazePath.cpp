...
int mazePath(int startRow,int startCol,int endRow,int endCol){
...
void ways(int startRow,int startCol,int endRow,int endCol,string s){
    if(startRow>endRow || startCol>endCol)return;
    if(startRow==endRow && startCol == endCol){
        cout << s << endl;
        return;
    }
    ways(startRow,startCol+1,endRow,endCol,s+'R');
    ways(startRow+1,startCol,endRow,endCol,s+'D');
}
