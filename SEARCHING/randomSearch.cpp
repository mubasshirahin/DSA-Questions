...
int randomSearchWithReplacement(vector<int>&v,int target,int size){
    srand(time(0));
    int idx = rand() % size;
    if(v[idx]==target){
        return idx;
...
int randomSearchWithoutReplacement(vector<int>&v,int target,int size){
    srand(time(0));
    vector<int> idx;
...
int main()
{
    ...
    vi v = {1,2,5,3,2,5,2,5};

    cout<<randomSearchWithoutReplacement(v,3,8);
